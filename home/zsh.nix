  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      test = "sudo nixos-rebuild test --flake ~/nix#$(hostname -s)";
			...
			rebuildclean = "nix flake update /home/lenny/nix && sudo nixos-rebuild switch --flake /home/lenny#$(${pkgs.hostname}/bin/hostname -s) --upgrade && sudo nix-collect-garbage -d && sudo nix-store --optimise";
			gyat = "git";
    };
    plugins = [
    {
